...
@Slf4j
public class BorderLeftStyleFactory extends AbstractStyleFactory {

    @Override
    public PoiStyle getStyle(Rule rule) {
        String color;
        Map<String, String> properties = rule.getValues();
        if (properties.containsKey("border-left-color")) {
            String colorProperty = properties.get("border-left-color");
            try {
        ...
        String width = properties.getOrDefault("border-left-width",Border.DEFAULT_STYLE);
        String style = properties.getOrDefault("border-left-style", Border.DEFAULT_STYLE);
        return BorderLeft.builder()
            .color(color)
            .width(width)
            .style(style)
            .build();
